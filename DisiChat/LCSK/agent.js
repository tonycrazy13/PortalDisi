...
$(function () {
    ...
    $(window).on('beforeunload', function (e) {
        if (agent.isOnline) {
            var msg = 'Por favor cambie su estado a "Fuera de Línea" antes de cerrar esta página. De lo contrario el visitante todavía será capaz de enviar la solicitud de chat para usted.';
            e.returnValue = msg;
            return msg;
        }
    });
    ...
    myHub.client.loginResult = function (status, id, name) {
        if (status) {
            ...
            var msg = id == 'config' ?
                'No se encontró el archivo de configuración. Por favor, configure Este Chat antes de usarlo' :
                'Ningún agente coincide con esas credenciales.';
    ...
    myHub.client.visitorSwitchPage = function (lastId, newId, newPage) {
        ...
        var session = [];
        session.push('<strong>system</strong> El visitante es de ahora: ' + newPage);
        ...
        if (lastId == getCurrentChatId()) {
            $('#chatmsgs' + lastId).append('<p><strong>system</strong> El visitante es de ahora: ' + newPage + '</p>');
